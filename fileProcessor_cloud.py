...
# Downloads a file from the to_process folder on Google Drive, processes the file to contain the correct information, 
# splits the file, then uploads into the to_email folder.
def process_files(processingFn=None, fileType='.csv',
  splitSizes=[], defaultSplitSize=1000, removeDuplicatesFn=None, processedFolderName='processed', togglePrint=True, emails=[]):
...
# Downloads files from "to_email" folder on Google Drive, and sends emails from that file.
def send_emails(from_email, fileType='.csv', processedFolderName="emailed", togglePrint=True, emails=[]):
  # get creds and refresh if needed.
  creds = google_get_creds()
  ...
  for fileName in os.listdir(emailFolderPath):
    ...
    if (os.path.isfile(filePath)):
      if (fileName.endswith(fileType)):
        ...
        studyfind_sendEmails(filePath, from_email)
...
def moveFileToFolder(filePath, folderPath, togglePrint=True):
  ...
  # move the new file into the folder. make sure the file exists.
  if (os.path.isfile(filePath)):
...
def splitCSV(creds, filePath, parentFolderPath, parentFolderId, splitSizes, defaultSize=None, togglePrint=True):
